...
object M3uParser {
    ...
    // Recognizes a leading season/episode code, such as as playlists that put
    // the code BEFORE the show name: "S01E04 Susuk Rupawan", "S1E4 Keturunan".
    // Tried before the suffix patterns below.
    private val seriesPrefixPattern = Regex("^S(\\d{1,2})E(\\d{1,3})[\\s._-]+(.+)$", RegexOption.IGNORE_CASE)
    ...
    fun parseSeriesInfo(title: String): Pair<String, Int?> {
        val prefixMatch = seriesPrefixPattern.find(title)
        if (prefixMatch != null) {
            val episode = prefixMatch.groupValues[2].toIntOrNull()
            val show = prefixMatch.groupValues[3].trim().trim('-', '.', ':', '_').trim()
